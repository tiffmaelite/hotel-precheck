...
GridView {
    ...
    //cellHeight: Math.floor(grid.innerHeigh / (grid.count/(grid.innerWidth/grid.cellWidth))) //hauteur divisée par le nombre nécessaire de lignes, c'est-à-dire le nombre d'éléments divisés par le nombre de colonnes (qui lui-même, étant variable, est calculé à partir de la largeur totale divisée par la largeur de chaque cellule)
    cellHeight: (grid.count > 0) ? Math.floor((grid.innerHeight*grid.innerWidth) / (grid.count*grid.cellWidth)) : 10 //idem que ci-dessus mais plus clair : aire disponible divisée par la largeur totale occupée par les cellules
    currentIndex: -1
    focus: true
