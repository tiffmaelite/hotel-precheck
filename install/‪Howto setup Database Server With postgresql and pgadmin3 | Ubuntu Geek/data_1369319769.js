...
{return;}}}}
this._logQS(this._buildQS(qps),o);},getTrackerUrl:function(av)
...
{qString+="&redir="+encodeURIComponent(redirect);}
var url=this._mkLogUrl(qString,{});return url;}});};$iTXT.js.loader["$iTXT.data.Advert"]=true;$iTXT.data.Advert_Load=function(){var undefined;$iTXT.data.AdvertManager={id:0,ads:{},remapped:{},add:function(ad)
{if(ad)
...
return null;}};$iTXT.data.Advert=$iTXT.core.Class.create({template:null,params:null,id:-1,did:0,liveLookupFinished:false,avSent:false,childAdverts:null,processed:false,init:function(template,params,attributes)
...
return rv;}});};$iTXT.js.loader["$iTXT.data.AdvertHandler"]=true;$iTXT.data.AdvertHandler_Load=function(){var undefined;$iTXT.data.AdvertHandler=$iTXT.core.Class.create({advert:null,callback:null,init:function(advert)
...
{var p=$iTXT.glob.params;p.set("normalisedcountry",p.get("cc"));p.set("realcountry",p.get("rcc"));p.set("region",p.get("reg"));p.set("pcode",p.get("postcode"));p.set("regname",p.get("regionname"));}});};$iTXT.js.loader["$iTXT.data.Context"]=true;$iTXT.data.Context_Load=function()
...
{oParams.visscanwholepage=!!('*'==this.params.get('visibilitynodes',[])[0]);}
...
{var nV=$itxtUtil.cleanString($iTXT.data.Context.textNodes[j].nodeValue);dbNodes+='<br />'+j+': '+((nV.length>72)?nV.substr(0,70)+'...':nV)+' ('+nV.length+')';}}}},getProcessedPercentage:function()
...
{}
rA[rA.length]=nd;}}
this.params.set("mintextlength",0);this.params.set("compatmode","v1");var hN=this.params.get("hooknodes",null)
{if(hN&&VM._.isArray(hN))
...
spliced.push(nId);tA[tI]={};}}};if(1==s.p)
...
{$iTXT.glob.params.set('refurl',this.getPseudoURL(vSec),100);var spliced=this.modifyTree();this.lastTime=(this.timings.cEnd-this.timings.cStart);if($iTXT.debug.Util.isLoggingOn())
{var sTree=$itxtUtil.serialiseJSON(this.tree,this.allowedFields);var json=(this.metrics&$iTXT.debug.Util.ECH_XPAND_JSON)?$itxtUtil.formatJSONString(sTree,true):sTree;}
if(this.metrics&$iTXT.debug.Util.ECH_TEXTNODES_DUMP)
{var hnl=$iTXT.data.Context.textNodes.length;if(hnl>0)
...
{var cS=this.getComputedStyles(elmt);return!!(cS.visibility!='hidden'&&cS.display!='none');},getDefinedStyle:function(elmt,style,defaultStyle,rejects){var retStyle=defaultStyle,found=false;;try{var retStyle=getComputedStyle(elmt,null)[style];found=true;if(rejects){if(rejects.length===1){if(retStyle===rejects[0]){found=false;}}else{for(var i=0,len=rejects.length;i<len;i++){if(retStyle===rejects[0]){found=false;}}}}}catch(e){return defaultStyle;}
if(!found){return $iTXT.data.Dom.getDefinedStyle(elmt.parentNode,style,defaultStyle,rejects);}else{return retStyle;}},propChangeIntervals:{},propChangeSubs:{},setOnPropChange:function(elmt,f)
...
{this.visibilityStates[visSec]=currState;f(elmt);return true;}}
return false;}};};$iTXT.js.loader["$iTXT.data.Param"]=true;$iTXT.data.Param_Load=function(){var undefined;var $itxtUtil=$iTXT.core.Util;$iTXT.data.Param=$iTXT.core.Class.create({paramHash:null,parent:null,debugName:null,init:function(p,params,weight,dbgName)
...
{return def;}
return value;},weigh:function(pname)
...
{var msg='ERROR attempting to defer for function "'+job+'" on iteration '+iteration+'.  Error message is "'+x.message+'".';$itxtUtil.dropScript('http://'+$iTXT.js.serverName+'/v3/errorlog.jsp?msg='+encodeURIComponent('checkBeeacons: '+msg),function(removeFunction){removeFunction();});}},drop:function(pixel,mode,confirm)
{try
...
{var msg='('+pixel.id+') ERROR attempting to drop Pixel "'+pixel.url+'" in mode "'+mode+'" with confirm '+confirm+'.  Error message is "'+x.message+'".';$itxtUtil.dropScript('http://'+$iTXT.js.serverName+'/v3/errorlog.jsp?msg='+encodeURIComponent('dropBeeacon: '+msg),function(removeFunction){removeFunction();});}},findActive:function(id)
