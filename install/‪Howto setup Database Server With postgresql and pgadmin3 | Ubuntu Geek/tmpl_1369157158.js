...
var contDiv=document.createElement("div");contDiv.id=this.options.id+"flashCont";var flashStyle="display: inline-block;";var fDC=this.advert.params.get("fl.div.col");if(null!=fDC)
{if('#'!=fDC.substr(0,1))
{fDC='#'+fDC;}
flashStyle+='background-color: '+fDC+';';}
this.flashStyleDiv=$iTXT.core.Builder.make("DIV",{id:this.options.id+"Style",style:flashStyle});this.flashHldr=$iTXT.core.Builder.make("DIV",{id:this.options.id+"Hldr",style:"display: inline-block;-moz-box-sizing: content-box;box-sizing: content-box;-ms-box-sizing: content-box;-webkit-box-sizing: content-box;"},[this.flashStyleDiv]);this.rootElement.itxtAppendChild(this.flashHldr);var flashProperties={width:this.properties.width||this.template.defaultWidth,height:this.properties.height||this.template.defaultHeight,id:this.options.id+"MC",name:this.options.id+"MC",src:actualSrc,style:"display: block",scale:"exactfit",quality:"high",wmode:"transparent",swliveconnect:"true",allowscriptaccess:"always",allowfullscreen:"true",flashvars:this._getFlashVars(),type:"application/x-shockwave-flash",bgcolor:"transparent"};this.finalFlashProperties=flashProperties;if($iTXT.core.Browser.is("Explorer"))
...
{var campaignSpecificFlParams=this.advert.params.get("fl.vars","").split(",");VM._(campaignSpecificFlParams).filter(function(flParam){return flParam!="";}).forEach(function(flParam){var flVal=this.advert.params.get("fl."+flParam,null);if(flVal)
